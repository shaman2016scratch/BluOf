...
const Credits = (props) => {
    ...
    return (
        <div class='main'>
            ...
            <p><b>{developers[1].name}</b> {developers[1].role} | { githubUser(developers[1]) } | { siteUser(developers[0]) }</p>
            <p><b>{developers[2].name}</b> {developers[2].role} | { githubUser(developers[2]) } | { siteUser(developers[0]) }</p>
            <h2>Helpers</h2>
            <p><b>{helpers[0].name}</b> {helpers[0].role} | { githubUser(helpers[0]) } | { siteUser(helpers[0]) }</p>
            <p><b>{helpers[1].name}</b> {helpers[1].role} | { githubUser(helpers[1]) } | { siteUser(helpers[0]) }</p>
		</div>
    );
};
